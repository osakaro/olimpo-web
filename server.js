require('dotenv').config(); 
const express = require('express');
const mysql = require('mysql2/promise');
const cors = require('cors');
const path = require('path');
const fs = require('fs').promises;

const app = express();
app.use(cors()); 
app.use(express.json()); 

// CONFIGURACIÓN DE BASE DE DATOS (TiDB Cloud)
const dbConfig = {
    host: process.env.TIDB_CLOUD_HOST,
    user: process.env.TIDB_CLOUD_USER,
    password: process.env.TIDB_CLOUD_PASSWORD,
    database: process.env.TIDB_CLOUD_DATABASE,
    port: process.env.TIDB_CLOUD_PORT,
    ssl: {
        minVersion: 'TLSv1.2',
        rejectUnauthorized: true
    }
};

// ARCHIVOS ESTÁTICOS (CSS, Imágenes, JS del cliente)
app.use(express.static(path.join(__dirname, 'public')));

// RUTA PRINCIPAL: Envía el XML original sin modificaciones
app.get('/', async (req, res) => {
    try {
        res.header('Content-Type', 'application/xml');
        res.sendFile(path.join(__dirname, 'public', 'hamburgueseria.xml'));
    } catch (error) {
        console.error("Error al cargar la web:", error);
        res.status(500).send("Error en el Servidor del Olimpo");
    }
});

// --- API: AUTENTICACIÓN Y GESTIÓN ---

// LOGIN: Verifica credenciales
app.post('/login', async (req, res) => {
    const { email, password } = req.body;
    let connection;
    try {
        connection = await mysql.createConnection(dbConfig);
        const [results] = await connection.execute(
            "SELECT nombre FROM usuarios WHERE email = ? AND password = ?", 
            [email, password]
        );
        
        if (results.length > 0) {
            res.json({ exito: true, nombre: results[0].nombre, mensaje: "BIENVENIDO" });
        } else {
            res.status(401).json({ exito: false, mensaje: "MORTAL NO ENCONTRADO" });
        }
    } catch (error) {
        res.status(500).json({ mensaje: "ERROR DB: " + error.message });
    } finally { if (connection) await connection.end(); }
});

// REGISTRO: Crea nuevos usuarios
app.post('/registro', async (req, res) => {
    const { nombre, email, password, direccion } = req.body;
    let connection;
    try {
        connection = await mysql.createConnection(dbConfig);
        await connection.execute(
            "INSERT INTO usuarios (nombre, email, password, direccion, puntos) VALUES (?, ?, ?, ?, 0)", 
            [nombre, email, password, direccion]
        );
        res.json({ mensaje: "MORTAL REGISTRADO" });
    } catch (error) {
        console.error(error);
        res.status(500).json({ mensaje: "ERROR AL REGISTRAR: " + error.message });
    } finally { if (connection) await connection.end(); }
});

// GUARDAR PEDIDO: Gestiona puntos y transacciones
app.post('/guardar-pedido', async (req, res) => {
    const { email, total, consumirPuntos, productos } = req.body;
    let connection;
    try {
        connection = await mysql.createConnection(dbConfig);
        
        // Lógica de puntos (1 punto por cada 10€, descontando envío si aplica)
        const totalComida = (total < 50 && total > 4.99) ? (total - 4.99) : total;
        const puntosGanados = Math.floor(totalComida / 10);
        
        const esDescuento = (consumirPuntos === true || consumirPuntos === 'si');
        const restaFija = esDescuento ? 10 : 0;

        // 1. Actualizar puntos del usuario
        await connection.execute(
            "UPDATE usuarios SET puntos = GREATEST(0, puntos + ? - ?) WHERE email = ?", 
            [puntosGanados, restaFija, email]
        );
        
        // 2. Insertar el registro del pedido
        await connection.execute(
            "INSERT INTO pedidos (usuario_email, productos, total, fecha) VALUES (?, ?, ?, NOW())", 
            [email, productos || 'Varios', total]
        );

        res.status(200).json({ mensaje: "PEDIDO GUARDADO Y PUNTOS ACTUALIZADOS" });
    } catch (error) {
        console.error(error);
        res.status(500).json({ mensaje: "ERROR AL PROCESAR PEDIDO: " + error.message });
    } finally { if (connection) await connection.end(); }
});

// HISTORIAL: Obtiene pedidos y saldo de puntos actual
app.get('/historial/:email', async (req, res) => {
    const email = req.params.email;
    let connection;
    try {
        connection = await mysql.createConnection(dbConfig);
        
        const [pedidos] = await connection.execute(
            "SELECT productos, total, DATE_FORMAT(fecha, '%d/%m/%Y %H:%i') as fecha_formateada FROM pedidos WHERE usuario_email = ? ORDER BY fecha DESC", 
            [email]
        );
        
        const [usuario] = await connection.execute(
            "SELECT puntos FROM usuarios WHERE email = ?", 
            [email]
        );
        
        const puntosReales = usuario.length > 0 ? usuario[0].puntos : 0;
        res.json({ pedidos, puntos: puntosReales });
    } catch (error) {
        res.status(500).json({ mensaje: "ERROR AL CARGAR HISTORIAL: " + error.message });
    } finally { if (connection) await connection.end(); }
});

// CONFIGURACIÓN DEL PUERTO
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
    console.log(`⚡ Servidor del Olimpo escuchando en el puerto ${PORT}`);
});